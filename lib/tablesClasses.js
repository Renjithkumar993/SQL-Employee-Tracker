...
class Department {
    ...
    deleteDept() {
        return new Promise((resolve, reject) => {
            ...
            db.query(`Delete from department WHERE id =${this.newDepartmenttoadd}`, function (err) {
                if (err) {
                    reject(err);
                } else {
                    resolve(console.log(`successfully deleted the department`));
                }
            });
...
class Role {
    ...
    deleteRole() {
        return new Promise((resolve, reject) => {
            ...
            db.query(`
            Delete from roles WHERE id =${this.title}`, function (err) {
                if (err) {
                    reject(err);
                } else {
                    resolve(console.log(`successfully deleted the Role`));
                }
            });
...
class Role1 extends Role {
    constructor(combinedsalaryid) {
        super()
        this.combinedsalaryid = combinedsalaryid
    }
    combinedSalary() {

        return new Promise((resolve, reject) => {
            db.query(`SELECT department.name AS Department, SUM(roles.salary) AS Total
            FROM roles
            JOIN department ON roles.department_id = ${this.combinedsalaryid}= department.id GROUP BY department.name`, function (err, result) {
                if (err) {
                    reject(err);
                } else {
                    resolve(console.table(result));
...
class Employee2 extends Employee {
    ...
    deleteEmployee() {
        return new Promise((resolve, reject) => {
            ...
            db.query(`Delete from employee WHERE id =${this.employeeUpdate}`, function (err) {
                if (err) {
                    reject(err);
                } else {
                    resolve(console.log(`successfully deleted the Employee`));
                }
            });
...
module.exports = { Department, Role, Employee, Employee1, Employee2, Employee3, Department1, Role1 };
